...
async function analyticsRoutes(fastify) {
    const analyticsService = new analytics_service_1.AnalyticsService(fastify.prisma);
    ...
    fastify.get('/overview', {
        ...
        try {
            ...
            const overview = await analyticsService.getOverview(tenantId, dateRange, propertyId);
    ...
    fastify.get('/occupancy', {
        ...
        try {
            const occupancy = await analyticsService.getOccupancy((0, utils_1.getTenantId)(request), dateRange, propertyId);
    ...
    fastify.get('/revenue', {
        ...
        try {
            const revenue = await analyticsService.getRevenue((0, utils_1.getTenantId)(request), dateRange, propertyId);
    ...
    fastify.get('/export', {
        ...
        try {
            const csvBuffer = await analyticsService.exportData((0, utils_1.getTenantId)(request), dateRange, propertyId);
            const filename = `analytics-${(0, date_fns_1.format)(new Date(), 'yyyy-MM-dd')}.csv`;
            return reply
