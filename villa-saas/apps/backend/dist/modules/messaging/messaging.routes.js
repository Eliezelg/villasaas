...
const createConversationSchema = zod_1.z.object({
    ...
    guestName: zod_1.z.string().min(1).optional(),
});
const sendMessageSchema = zod_1.z.object({
    content: zod_1.z.string().min(1),
    type: zod_1.z.enum(['TEXT', 'IMAGE', 'VIDEO', 'AUDIO', 'FILE', 'LOCATION']).default('TEXT'),
    attachments: zod_1.z.array(zod_1.z.object({
        type: zod_1.z.enum(['IMAGE', 'VIDEO', 'AUDIO', 'DOCUMENT', 'OTHER']),
        name: zod_1.z.string(),
        url: zod_1.z.string(),
        size: zod_1.z.number(),
...
async function messagingRoutes(fastify) {
    ...
    fastify.get('/conversations', {
        ...
        const { status = 'ACTIVE', propertyId, page = 1, limit = 20 } = request.query;
        const where = {
            tenantId,
            status,
            participants: {
    ...
    fastify.post('/conversations', {
        ...
        if (validatedData.propertyId) {
            ...
            tenantId = (0, utils_1.getTenantId)(request);
        }
        else {
    ...
    fastify.patch('/conversations/:id', {
        ...
        const tenantId = (0, utils_1.getTenantId)(request);
        const { id } = request.params;
        const { status, priority, tags } = request.body;
        const conversation = await fastify.prisma.conversation.findFirst({
            where: {
                id,
                tenantId,
