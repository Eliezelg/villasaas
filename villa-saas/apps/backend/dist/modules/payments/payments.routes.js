...
const createPaymentIntentSchema = zod_1.z.object({
    ...
    metadata: zod_1.z.object({
        ...
        infants: zod_1.z.number().optional(),
        pets: zod_1.z.number().optional(),
...
async function paymentsRoutes(fastify) {
    ...
    fastify.get('/stripe/connect/status', {
        ...
        try {
            ...
            const needsUpdate = tenant.stripeDetailsSubmitted !== account.details_submitted ||
    ...
    fastify.get('/stripe/connect/oauth/callback', async (request, reply) => {
        const { code, state, error: stripeError } = request.query;
    ...
    fastify.post('/public/payments/create-intent', {
        ...
        try {
            ...
            const bookingService = new (await Promise.resolve().then(() => __importStar(require('../bookings/booking.service')))).BookingService(fastify.prisma);
            // Parser les dates
            ...
            const priceCalculation = await bookingService.calculateBookingPrice(metadata.propertyId, checkIn, checkOut, guests, metadata.selectedOptions);
                                ...
                                setTimeout(async () => {
                                    try {
                                        ...
                                        if (currentBooking && currentBooking.paymentStatus === 'FAILED' && currentBooking.status === 'PENDING') {
                                            ...
                                            const emailServiceForCancellation = (0, email_service_1.createEmailService)(fastify);
