...
async function propertyImageS3Routes(fastify) {
    ...
    fastify.put('/:propertyId/images/:imageId/primary', {
        ...
        const updatePromises = otherImages.map((img, index) => fastify.prisma.propertyImage.update({
            where: { id: img.id },
            data: { order: index + 1 },
        }));
    ...
    fastify.put('/:propertyId/images/order', {
        ...
        await fastify.prisma.$transaction(async (tx) => {
            ...
            const tempUpdatePromises = images.map((img, index) => tx.propertyImage.update({
                where: { id: img.id },
                data: { order: -(index + 1) }, // Valeurs négatives temporaires
            }));
            await Promise.all(tempUpdatePromises);
            // Ensuite, mettre à jour avec les vraies valeurs
            const finalUpdatePromises = images.map(img => tx.propertyImage.update({
                where: { id: img.id },
                data: {
                    order: img.order,
